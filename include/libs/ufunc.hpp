...
namespace numcpp {
        ...
        for (ll_t i = 0; i < arr_shape.rows; i++) {
            for (ll_t j = 0; j < arr_shape.cols; j++) {
                result[idx++] = !where || (*where)[broadcast_index({i, j}, where_shape)]
                    ? func(static_cast<T>(arr[{i, j}]), args...)
                    : dtype(0);
            }
        }
    ...
    array<dtype> ufunc_binary(const array<L>& lhs, const array<R>& rhs, out_t<dtype> out, const where_t& where,
        ...
        for (ll_t i = 0; i < res_shape.rows; i++) {
            for (ll_t j = 0; j < res_shape.cols; j++) {
                result[idx++] = !where || (*where)[broadcast_index({i, j}, where_shape)]
                    ? func(static_cast<L>(lhs[broadcast_index({i, j}, lhs_shape)]),
                           static_cast<R>(rhs[broadcast_index({i, j}, rhs_shape)]), args...)
                    : dtype(0);
            }
        }
    ...
    template <typename T, typename dtype, typename Func, typename... Args>
    array<dtype> ufunc_axes_unary(const array<T>& arr, const int axis, out_t<dtype> out, const bool keepdims, Func func,
                                  Args... args) {
    ...
    template <typename L, typename R, typename dtype, typename Func, typename... Args>
    array<dtype> ufunc_axes_binary(const array<L>& lhs, const array<R>& rhs, const int8_t axis, out_t<dtype> out,
                                   const bool keepdims, Func func, Args... args) {
        ...
        if (axis == 0 || axis == -2) {
            ...
            for (ll_t i = 0; i < col; i++) {
                buf[i] = func(lhs[{slice_t(), i}], rhs[{slice_t(), i}], args...);
            }
            array<dtype> res(std::move(buf), col);
            return keepdims ? res[slice_t()] : res;
        }
